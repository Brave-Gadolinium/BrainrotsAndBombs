...
local function OnChildRemoved(child: Instance): ()
	...
	local slot: any = SlotsByTool[tool]
	if slot then
		local removedIndex = slot.Index 
		slot:Clear()

		if slot.Index > NumberOfHotbarSlots then
			slot:Delete()
		elseif not InventoryFrame.Visible then
			for i = removedIndex + 1, NumberOfHotbarSlots do
				local nextSlot = Slots[i]
				local targetSlot = Slots[i - 1]

				if nextSlot and nextSlot.Tool and targetSlot and not targetSlot.Tool then
					nextSlot:Swap(targetSlot)
				end
			end
			AdjustHotbarFrames()
		end
